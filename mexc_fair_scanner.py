...
async def tg_get_updates(session: aiohttp.ClientSession, offset: Optional[int]) -> Dict[str, Any]:
    ...
    async with session.get(url, params=params, timeout=aiohttp.ClientTimeout(total=POLL_UPDATES_TIMEOUT + 10)) as r:
        return await r.json(content_type=None)
...
async def arb_loop(session: aiohttp.ClientSession, store: Dict[str, Any], settings_lock: asyncio.Lock):
    ...
    while True:
        ...
        try:
            ...
            mexc, bybit, binance, okx, kucoin, gate = await asyncio.gather(
                mexc_task, bybit_task, binance_task, okx_task, kucoin_task, gate_task
            )
